...
const allNotifications: Notification[] = [
  ...
  },
  {
    ...
    message: 'Ada 3 percobaan login gagal ke akun HR Manager (IP: 192.168.1.10).',
...
export default function NotificationPage() {
  ...
  return (
    <div className="min-h-screen px-4 py-8 text-zinc-900 dark:text-white">
      <main className="w-full mx-auto">
        <div className="mb-8 pl-2">
          <h2 className="text-zinc-800 dark:text-white font-extrabold text-3xl">Selamat Siang!</h2>
          
          <div className="mt-4">
            <h3 className="font-semibold text-black dark:text-white text-xl">Notifikasi</h3>
            <p className="text-sm text-gray-700 dark:text-gray-300">
              Anda memiliki {allNotifications.filter(n => n.status === 'unread').length} notifikasi untuk dilihat
            </p>
          </div>
        ...
        <div className="space-y-8 w-full">
          {groups.map(group => {
            const notifs = allNotifications.filter(
              n => n.group === group && (filter === 'semua' || n.status === 'unread')
            );
                  ...
                  {notifs.map(n => (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center">
                        <div className="flex items-center gap-4"> {/* Changed to flex with items-center and gap-4 */}
